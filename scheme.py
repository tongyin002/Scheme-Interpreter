...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        operands = lambda expr : scheme_eval(expr, env)
        return scheme_apply(operator, rest.map(operands), env)
        # END PROBLEM 5
...
def eval_all(expressions, env):
    ...
    temp = expressions.second
    while temp is not nil:
        result = scheme_eval(temp.first, env)
        temp = temp.second
    ...
    def make_child_frame(self, formals, vals):
        ...
        if len(formals) != len(vals):
            ...
            while temp_formal is not nil:
                frame.define(temp_formal.first, temp_val.first)
                temp_formal = temp_formal.second
                temp_val = temp_val.second
            return frame
        # END PROBLEM 11
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        temp = LambdaProcedure(formals, body, env)
        env.define(target.first, temp)
        return target.first
        # END PROBLEM 10
    else:
